...
describe(`getPotentialAncestors`, function() {
  it(`works`, function() {
    ...
    expect(ancestors.get(types.DeviceConnection).fields.has('pageInfo')).to.be
      .false
    expect(ancestors.get(types.Organization)).to.exist
    expect(ancestors.get(types.Organization).fields.has('Devices')).to.be.true
    expect(ancestors.get(types.Organization).fields.has('DeviceGroups')).to.be
      .true
    expect(ancestors.get(types.Organization).fields.has('Users')).to.be.true
    expect(ancestors.get(types.Organization).fields.has('UserGroups')).to.be
      .true
